...
public class Player : MonoBehaviour
{
    ...
    void Move()
    {
        ...
        if(!isBorder) // 회전이 제한되는 것을 막기 위해 이동만 제한하도록 움직이기 전에 조건을 달아줌
    ...
    void Grenade()
    {
        ...
        if(gDown && !isReload && !isSwap && !isDead)
        {
            ...
            if (Physics.Raycast(ray, out rayHit, 100f))
            {
                ...
                GameObject instGrenade = Instantiate(grenadeObj, transform.position, transform.rotation);
    ...
    void Swap()
    {
        if(sDown1 && (!hasWeapons[0] || equipWeaponIndex == 0)) // 1번을 눌렀을 때, 무기를 가지고 있지 않거나 이미 들고 있다면 return
            return;
        ...
        {
            if(equipWeapon != null) // 이미 무기를 장착 중이면
                equipWeapon.gameObject.SetActive(false);

            equipWeaponIndex = weaponIndex; // 현재 무기 번호 대입
    ...
    void Interaction()
    {
        if(iDown && nearObject != null && !isJump && !isDodge && !isDead)
        {
            if(nearObject.tag == "Weapon")
            {
                ...
                hasWeapons[weaponIndex] = true; // 해당 무기 소지 체크
